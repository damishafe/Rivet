...
- receipt `4893ed98936e3ae6`
- **passed: False**
- export pack: withheld
- failing checks (every scene, in all three formats):
  - `hook` **A01** — mismatch
  - `proof` **A01** — mismatch
  - `cta` **A01** — mismatch
...
The same scene as a vertical story, a square feed post and a wide banner. Each carries its
own composition rather than a squashed crop, and each is audited separately — a legible
story does not smuggle through an illegible banner.
